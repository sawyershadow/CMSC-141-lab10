...
p5.prototype.intervalCurrentPosition = function(
  callback,
  interval,
  errorCallback
) {
...
p5.prototype.isLocationInPolygon = function(poly, pt) {
  for (var c = false, i = -1, l = poly.length, j = l - 1; ++i < l; j = i)
    ((poly[i].lon <= pt.lon && pt.lon < poly[j].lon) ||
      (poly[j].lon <= pt.lon && pt.lon < poly[i].lon)) &&
      pt.lat <
        ((poly[j].lat - poly[i].lat) * (pt.lon - poly[i].lon)) /
 ...
 * @param  {object} an positionOptions object: enableHighAccuracy, maximumAge, timeout
 */
p5.prototype.geoFenceCircle = function(
  lat,
  lon,
  fence,
  insideCallback,
...
p5.prototype.geoFencePolygon = function(
  ArrayOfObjectsWithLatLong,
  insideCallback,
  outsideCallback,
  units,
  options
